...
>(({ onOpenChange: setOpenProp, defaultOpen = false, open: openProp, className, children, style, ...props }, ref) => {
...
function useHoverIntentForSidebar(opts: {
  isMobile:      boolean;
  open:          boolean;
  ...
  const { isMobile, open, setOpen, openMobile, setOpenMobile } = opts;
  ...
  const onEnter = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      if(!openT.current) openT.current = window.setTimeout(() => { 
        openT.current = null; 
        setOpen(true); 
      }, OPEN);
    },
  ...
  const onLeave = React.useCallback(
    (e: React.MouseEvent) => {
      ...
      if(!closeT.current) closeT.current = window.setTimeout(() => { 
        closeT.current = null; 
        setOpen(false); 
      }, CLOSE);
    },
  ...
  const explicitToggle = React.useCallback(() => {
    clear();
  ...
  }, [clear, isMobile, openMobile, setOpen, setOpenMobile]);

  return { ref, onEnter, onLeave, onFocus, onBlur, clear, explicitToggle };
}
...
>(({ 
  ...
  const { ref: wrapRef, onEnter, onLeave, onFocus, onBlur, clear } = useHoverIntentForSidebar({
    isMobile,
    open,
    setOpen,
    openMobile,
  ...
  ({ className, asChild = false, ...props }, ref) => {
    ...
    return (
      <Comp
        ...
        className={cn(
          "absolute right-3 top-3.5 p-8 flex aspect-square w-5 items-center justify-center rounded-md text-sidebar-foreground outline-none ring-sidebar-ring transition-transform hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
>(({ asChild = false, isActive = false, size = "default", variant = "default", className, tooltip, ...props }, ref) => {
  const Comp = asChild ? Slot : "button";
  const { isMobile, state } = useSidebar();

  const button = (
    <Comp ref={ref} data-size={size} data-active={isActive} data-sidebar="menu-button" className={cn(sidebarMenuButtonVariants({ variant, size }), className)} {...props} />
  );
